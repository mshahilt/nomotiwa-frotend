...
function User() {
  ...
  const fetchDoctors = async () => {
    try {
      ...
      // Initialize tokens for all doctors
      const initialTokens = {};
      data.forEach(doctor => {
  ...
  useEffect(() => {
    // Join rooms for all doctors
    doctors.forEach(doctor => {
      // Join room with the format doctor_${doctorId}
      socket.emit("joinDoctorRoom", `doctor_${doctor._id}`);
    });

    // Listen for token updates
    socket.on("tokenUpdate", (data) => {
      console.log("Received token update:", data);
      // Update token based on the current doctor's room
      if (data.currentToken) {
        const doctorId = data.doctorId;
        setDoctorTokens(prev => ({
          ...prev,
          [doctorId]: data.currentToken
        }));
      }
    });
    ...
    return () => {
      ...
      doctors.forEach(doctor => {
        socket.emit("leaveDoctorRoom", `doctor_${doctor._id}`);
      });
    };
  }, [doctors]);
              ...
              <div className="flex items-center mt-4 pt-4 border-t border-gray-100">
                <div className="flex items-center space-x-2">
                  <div className={`w-2 h-2 rounded-full ${
                    !doctorTokens[doctor._id] || doctorTokens[doctor._id] === "Waiting..." 
                      ? "bg-yellow-400" 
                      : "bg-green-400"
                  }`}></div>
